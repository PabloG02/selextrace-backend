...
/// Implements the `CapR.cpp` and `CapR.h` of the CapR package available at
...
public class CapR {
    ...
    private int _maximal_span;

    private final double[] hairpin = new double[31];
    private final double[][][] mismatchH = new double[7][5][5];
    private final double[][][] mismatchI = new double[7][5][5];
    private final double[][] stack = new double[7][7];
    private final double[] bulge = new double[31];
    private double TermAU;
    private final double[][][][] int11 = new double[8][8][5][5];
    private final double[][][][][] int21 = new double[8][8][5][5][5];
    private final double[][][][][][] int22 = new double[8][8][5][5][5][5];
    private final double[] internal = new double[31];
    private double MLclosing;
    private double MLintern;
    private double MLbase;
    private final double[][] dangle5 = new double[8][5];
    private final double[][] dangle3 = new double[8][5];
    private final double[] ninio = new double[EnergyPar.MAXLOOP + 1];

    private final ArrayList<Integer> _int_sequence = new ArrayList<>();
    private int _seq_length;

    private final ArrayList<Double> _Alpha_outer = new ArrayList<>();
    private final DataMatrix<Double> _Alpha_stem = new DataMatrix<>();
    private final DataMatrix<Double> _Alpha_stemend = new DataMatrix<>();
    private final DataMatrix<Double> _Alpha_multi = new DataMatrix<>();
    private final DataMatrix<Double> _Alpha_multibif = new DataMatrix<>();
    private final DataMatrix<Double> _Alpha_multi1 = new DataMatrix<>();
    private final DataMatrix<Double> _Alpha_multi2 = new DataMatrix<>();

    private final ArrayList<Double> _Beta_outer = new ArrayList<>();
    private final DataMatrix<Double> _Beta_stem = new DataMatrix<>();
    private final DataMatrix<Double> _Beta_stemend = new DataMatrix<>();
    private final DataMatrix<Double> _Beta_multi = new DataMatrix<>();
    private final DataMatrix<Double> _Beta_multibif = new DataMatrix<>();
    private final DataMatrix<Double> _Beta_multi1 = new DataMatrix<>();
    private final DataMatrix<Double> _Beta_multi2 = new DataMatrix<>();
    ...
    private void set_energy_parameters() {
        MLclosing = -EnergyPar.ML_closing37 * 10.0 / EnergyPar.kT;
        MLintern = -EnergyPar.ML_intern37 * 10.0 / EnergyPar.kT;
        MLbase = -EnergyPar.ML_BASE37 * 10.0 / EnergyPar.kT;
        TermAU = -EnergyPar.TerminalAU * 10.0 / EnergyPar.kT;

        for (int i = 0; i <= 30; i++) {
            hairpin[i] = -EnergyPar.hairpin37[i] * 10.0 / EnergyPar.kT;
            bulge[i] = -EnergyPar.bulge37[i] * 10.0 / EnergyPar.kT;
            internal[i] = -EnergyPar.internal_loop37[i] * 10.0 / EnergyPar.kT;
        }
        ...
        for (int i = 0; i < 7; i++) {
            ...
            for (int j = 0; j < 7; j++) {
                stack[i][j] = -EnergyPar.stack37[i][j] * 10.0 / EnergyPar.kT;
            }

            for (int j = 0; j <= 4; j++) {
                dangle5[i][j] = -EnergyPar.dangle5_37[i][j] * 10.0 / EnergyPar.kT;
                dangle3[i][j] = -EnergyPar.dangle3_37[i][j] * 10.0 / EnergyPar.kT;
                if (i > 2) {
                    dangle3[i][j] += TermAU;
                }
            }
        ...
        for (int i = 0; i <= 7; i++) {
            for (int j = 0; j <= 7; j++) {
                for (int k = 0; k < 5; k++) {
                    for (int l = 0; l < 5; l++) {
                        int11[i][j][k][l] = -InitLoops.int11_37[i][j][k][l] * 10.0 / EnergyPar.kT;
                        for (int m = 0; m < 5; m++) {
                            int21[i][j][k][l][m] = -InitLoops.int21_37[i][j][k][l][m] * 10.0 / EnergyPar.kT;
                            for (int n = 0; n < 5; n++) {
                                int22[i][j][k][l][m][n] = -InitLoops.int22_37[i][j][k][l][m][n] * 10.0 / EnergyPar.kT;
                            }
                        }
                    }
                }
        ...
        for (int i = 0; i <= EnergyPar.MAXLOOP; i++) {
            ninio[i] = -Math.min(EnergyPar.MAX_NINIO, i * EnergyPar.F_ninio37) * 10.0 / EnergyPar.kT;
        }
    }
            ...
            for (int i = j - EnergyPar.TURN; i >= Math.max(0, j - _maximal_span - 1); i--) {
                ...
                if (type != 0) {
                    ...
                    if (_Alpha_stemend.get(i + 1, j - i - 2) != -EnergyPar.INF) {
                        // Stem¨StemEnd
                        temp = (flag) ? logsumexp(temp, _Alpha_stemend.get(i + 1, j - i - 2))
                                : _Alpha_stemend.get(i + 1, j - i - 2);
                        flag = true;
                    }

                    _Alpha_stem.set(i, j - i, (!flag) ? -EnergyPar.INF : temp);
                } else {
                ...
                for (int k = i + 1; k <= j - 1; k++) {
                    if (_Alpha_multi1.get(i, k - i) != -EnergyPar.INF
                        ...
                        temp = (!flag) ? _Alpha_multi1.get(i, k - i) + _Alpha_multi2.get(k, j - k)
                                : logsumexp(temp, _Alpha_multi1.get(i, k - i) + _Alpha_multi2.get(k, j - k));
                        flag = true;
                    }
                }
                _Alpha_multibif.set(i, j - i, (!flag) ? -EnergyPar.INF : temp);
                ...
                if (_Alpha_multi2.get(i, j - i - 1) != -EnergyPar.INF) {
                    ...
                    _Alpha_multi2.set(i, j - i, (!flag) ? -EnergyPar.INF : temp);
                }
    ...
    /// Retrieves the structural profile for the sequence in accordance with the
    /// `StructurePool` interface.
    ///
    /// The profile represents the probabilities of each nucleotide participating in
    /// various secondary structure motifs. The returned flat array is of size
    /// `5 * n` (where `n` is the sequence length), grouping the probabilities
    /// by structural motif.
    /// @return A `double[]` containing the structural probabilities formatted as:
    /// `[h_1...h_n, i_1...i_n, b_1...b_n, m_1...m_n, d_1...d_n]`.
    ///
    /// The prefixes denote the structural motif for each position:
    ///
    ///   - `h`: Hairpin loop
    ///   - `i`: Inner loop
    ///   - `b`: Bulge loop
    ///   - `m`: Multi-loop
    ///   - `d`: Dangling end (exterior loop)
    public double[] getStructuralProfile() {
        ...
        if (pf >= -690 && pf <= 690) {
            CalcBulgeAndInternalProbability2(profile, _seq_length, 2 * _seq_length);
        } else {
            CalcLogSumBulgeAndInternalProbability2(profile, _seq_length, 2 * _seq_length);
        }

        CalcHairpinProbability2(profile, 0);
    ...
    public void CalcStructuralProfile(String name) {
        ArrayList<Double> bulge_probability = new ArrayList<>(_seq_length);
        ArrayList<Double> internal_probability = new ArrayList<>(_seq_length);
        ArrayList<Double> hairpin_probability = new ArrayList<>(_seq_length);
        ArrayList<Double> multi_probability = new ArrayList<>(_seq_length);
        ArrayList<Double> exterior_probability = new ArrayList<>(_seq_length);
        ArrayList<Double> stem_probability = new ArrayList<>(_seq_length);
        ...
        sb.append("Bulge ");
        for (double value : bulge_probability) {
            sb.append(value).append(' ');
        }
        sb.append(System.lineSeparator());
        sb.append("Exterior ");
        for (double value : exterior_probability) {
            sb.append(value).append(' ');
        }
        sb.append(System.lineSeparator());
        sb.append("Hairpin ");
        for (double value : hairpin_probability) {
            sb.append(value).append(' ');
        }
        sb.append(System.lineSeparator());
        sb.append("Internal ");
        for (double value : internal_probability) {
            sb.append(value).append(' ');
        }
        sb.append(System.lineSeparator());
        sb.append("Multibranch ");
        for (double value : multi_probability) {
            sb.append(value).append(' ');
        }
//		sb.append(System.lineSeparator());
//		sb.append("Stem ");
...
//		System.out.println(sb.toString());
    ...
    private double CalcExteriorProbability(int x) {
        return (Math.exp(_Alpha_outer.get(x - 1) + _Beta_outer.get(x) - _Alpha_outer.get(_seq_length)));
    }
            ...
            for (int i = Math.max(1, x - _maximal_span); i < x; i++) {
                for (int j = x + 1; j <= Math.min(i + _maximal_span, _seq_length); j++) {
                    ...
                    if (_Beta_stemend.get(i, j - i - 1) != -EnergyPar.INF) {
                        ...
                        temp = flag ? logsumexp(temp, h_energy) : h_energy;
                        flag = true;
                    }
                }
            }

            profile[hairpin_offset + x - 1] = flag
                    ? Math.exp(temp - _Alpha_outer.get(_seq_length))
                    : 0.0;
        }
    }
    ...
    private double CalcMultiProbability(int x) {
        ...
        for (int i = x; i <= Math.min(x + _maximal_span, _seq_length); i++) {
            if (_Beta_multi.get(x - 1, i - x + 1) != -EnergyPar.INF
                ...
                temp = (!flag) ? _Beta_multi.get(x - 1, i - x + 1) + _Alpha_multi.get(x, i - x)
        ...
        for (int i = Math.max(0, x - _maximal_span); i < x; i++) {
            if (_Beta_multi2.get(i, x - i) != -EnergyPar.INF
                ...
                temp = (!flag) ? _Beta_multi2.get(i, x - i) + _Alpha_multi2.get(i, x - i - 1)
    ...
    private void CalcLogSumBulgeAndInternalProbability2(double[] profile, int bulge_offset, int internal_offset) {
        ...
        ArrayList<Boolean> b_flag_array = new ArrayList<>(_seq_length);
        ArrayList<Boolean> i_flag_array = new ArrayList<>(_seq_length);
        for (int x = 0; x < _seq_length; x++) {
                ...
                if (p != 0 && q != _seq_length) {
                    ...
                    for (int k = q + 1; k <= Math.min(_seq_length, p + _maximal_span); k++) {
                        if (_Beta_multibif.get(p, k - p) != -EnergyPar.INF
                            ...
                            temp = (!flag) ? _Beta_multibif.get(p, k - p) + _Alpha_multi2.get(q, k - q)
                                    : logsumexp(temp,
                                    _Beta_multibif.get(p, k - p) + _Alpha_multi2.get(q, k - q));
                            flag = true;
                        }
                    }
                    _Beta_multi1.set(p, q - p, (flag) ? temp : -EnergyPar.INF);
                    ...
                    if (q - p <= _maximal_span) {
                        if (_Beta_multi2.get(p, q - p + 1) != -EnergyPar.INF) {
                            temp = (flag) ? logsumexp(temp, _Beta_multi2.get(p, q - p + 1) + MLbase)
                    ...
                    for (int k = Math.max(0, q - _maximal_span); k < p; k++) {
                        if (_Beta_multibif.get(k, q - k) != -EnergyPar.INF
                            ...
                            temp = (!flag) ? _Beta_multibif.get(k, q - k) + _Alpha_multi1.get(k, p - k)
                                    : logsumexp(temp,
                                    _Beta_multibif.get(k, q - k) + _Alpha_multi1.get(k, p - k));
                            flag = true;
                        }
                    }
                    _Beta_multi2.set(p, q - p, (!flag) ? -EnergyPar.INF : temp);
    ...
    private double logsumexp(double x, double y) {
        return x > y
                ? x + Math.log(Math.exp(y - x) + 1.0)
                : y + Math.log(Math.exp(x - y) + 1.0);
    }
    ...
    private double LoopEnergy(int type, int type2, int i, int j, int p, int q) {
        ...
        // 2. Bulge loop
            ...
            z = (u <= 30)
                    ? bulge[u]
                    : bulge[30] - EnergyPar.lxc37 * Math.log(u / 30.0) * 10.0 / EnergyPar.kT;
        ...
        // 4. 1x2 Internal loop
            ...
            z = int21[type][type2][_int_sequence.get(i + 1)][_int_sequence.get(q + 1)][_int_sequence.get(j - 1)];

        // 5. 2x1 Internal loop
        } else if (u1 == 2 && u2 == 1) {
            z = int21[type2][type][_int_sequence.get(q + 1)][_int_sequence.get(i + 1)][_int_sequence.get(p - 1)];

        // 6. 2x2 Internal loop
        } else if (u1 == 2 && u2 == 2) {
            z = int22[type][type2][_int_sequence.get(i + 1)][_int_sequence.get(p - 1)][_int_sequence.get(q + 1)][_int_sequence.get(j - 1)];

        // 7. Generic internal loop (Fallback)
        } else {
            z = internal[u1 + u2]
                    + mismatchI[type][_int_sequence.get(i + 1)][_int_sequence.get(j - 1)]
                    + mismatchI[type2][_int_sequence.get(q + 1)][_int_sequence.get(p - 1)]
                    + ninio[Math.abs(u1 - u2)];
        }
    ...
    private double HairpinEnergy(int type, int i, int j) {
        ...
        double q = (d <= 30)
                ? hairpin[d]
                : hairpin[30] - EnergyPar.lxc37 * Math.log(d / 30.0) * 10.0 / EnergyPar.kT;
